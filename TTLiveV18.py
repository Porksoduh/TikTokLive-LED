...
GPIO.add_event_detect(button, GPIO.FALLING, callback=ledoff, bouncetime=2000)


#apply a collor ot each section of the printer
def tricolor(bot, mid, top):
    global botxy, midsl, midsr, midzl, midzr, topz, tops, tops1, midsr1
    for b in botxy:
        pixels[b] = bot
    for msl in midsl:
        pixels[msl] = mid
...
async def on_connect(_: ConnectEvent):
    strobe(datetime.timedelta(seconds=3), (255,0,255))
...
async def on_like(event: LikeEvent):
    global likesthres
    if event.totalLikeCount > likesthres:
...
async def on_connect(error: Exception):
    ...
    elif isinstance(error, FailedConnection):
        if failcount < failthreshold:
            time.sleep(5)
            await client.start()
            return
